...
export const api = {
     ...
     *
     * @param {string} streamId
     * @returns {Promise<{status: number; totalConnections: number; maxTotalConnections: number; throughputIn: number; throughputOut: number}>}
     */
    async stats(streamId) {
        ...
        const r = res_1?.response
        // Prefer the most recent throughput window; fall back to the lifetime
        // average when the last window is empty.
        const throughputIn = r?.lastThroughputIn || r?.avgThroughputIn || 0
        const throughputOut = r?.lastThroughputIn ? r?.lastThroughputOut : r?.avgThroughputOut
        return {
            status: res.status,
            totalConnections: r?.connections?.llhls || 0,
            maxTotalConnections: r?.maxTotalConnections || 0,
            throughputIn,
            throughputOut: throughputOut || 0,
        }
    },
    /**
     * 
