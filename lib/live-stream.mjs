...
const pubkeyLgm = process.env.PUBKEY_LGM;
...
export class LiveStream {
    ...
    /**
     * Lowest llhls connection count seen so far. OME keeps phantom LL-HLS
     * connections open (0 bytes out) that never drop below a floor, so this
     * tracks that floor and viewers are reported relative to it.
     * @type {number|null}
     */
    viewerBaseline = null
     ...
     * @param {string} id
     */
    constructor(pubkeyOrName) {
        if (pubkeyOrName.startsWith('local_') || pubkeyOrName.startsWith('remote_') || pubkeyOrName.startsWith('lgm_')) {
            this.id = pubkeyOrName.replace('local_', '').replace('remote_', '').replace('lgm_', '');
            this.name = pubkeyOrName
        } else {
            this.id = LiveStream.generateStreamId();
            this.pubkey = pubkeyOrName;
            if (this.pubkey === pubkeyLocal) {
                this.name = `local_${this.id}`;
            } else if (this.pubkey === pubkeyLgm) {
                this.name = `lgm_${this.id}`;
            } else {
    ...
    async waitForStreamReady(maxAttempts = 10, intervalMs = 1000) {
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            const { status } = await ome.api.stats(this.name)
            if (status === 200) {
                console.log(`Stream ${this.name} ready after ${attempt} attempt(s)`)
                return true
            }
            console.log(`Waiting for stream ${this.name}... attempt ${attempt}/${maxAttempts}`)
            await delay(intervalMs)
        }
        throw new Error(`Stream ${this.name} not ready after ${maxAttempts} attempts`)
    }
    ...
    async updateViewers() {
        ...
        if (!isLive) return;
        const { maxTotalConnections: rawMax, totalConnections: rawConnections } = await ome.api.stats(this.name)
        // Subtract OME's phantom LL-HLS connection floor (see viewerBaseline).
        this.viewerBaseline = this.viewerBaseline == null ? rawConnections : Math.min(this.viewerBaseline, rawConnections)
        const currentParticipants = Math.max(0, rawConnections - this.viewerBaseline)
        const totalParticipants = Math.max(0, rawMax - this.viewerBaseline)
        console.log("Update Viewers Stream ID:", this.name, { rawConnections, rawMax, baseline: this.viewerBaseline, currentParticipants, totalParticipants })
    ...
    async start() {
        ...
        const nostrEvent = {
            ...
            "tags": [
                ...
                ...tags.filter(t => !!t?.trim()).map(t => ['t', t]),
    ...
    async end() {
        ...
        evt.tags = [...newTags, ['ends', Math.floor(Date.now() / 1000).toString()], ["status", "ended"], ["recording", `${ome.url.recordings(this.name)}`]]
