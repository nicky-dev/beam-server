...
export class LiveStream {
    ...
    /**
     * Running peak of the estimated concurrent viewers, published as
     * `total_participants`.
     * @type {number}
     */
    maxViewers = 0
    ...
    async updateViewers() {
        ...
        if (!isLive) return;
        // OME's LLHLS connection/session count is unreliable (sessions accumulate
        // and never expire), so estimate viewers from egress: for a single bypass
        // rendition, throughputOut ≈ viewers × throughputIn.
        const { throughputIn, throughputOut } = await ome.api.stats(this.name)
        const currentParticipants = throughputIn > 0 ? Math.round(throughputOut / throughputIn) : 0
        this.maxViewers = Math.max(this.maxViewers, currentParticipants)
        const totalParticipants = this.maxViewers
        console.log("Update Viewers Stream ID:", this.name, { throughputIn, throughputOut, currentParticipants, totalParticipants })
